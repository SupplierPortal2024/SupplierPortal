...
public partial class purchase_purchase_request2 : System.Web.UI.Page
{
    ...
    protected string showexpired = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        this.showexpired = AXRequest.GetQueryString("showexpired");
        ...
        if (AXRequest.GetQueryString("showexpired") == "")
        {
            this.showexpired=this.ddlExpired.SelectedValue;
        }
        ...
        if (!Page.IsPostBack)
        {
            ...
            //(new EpicorRequest()).GetEpicorRFQRFHead(this.vendor_id);

            RptBind("id>0 " + CombSqlTxt(this.status, this.note_no, this.start_time, this.stop_time,this.showexpired), "RFQHead_Company,Vendor_Name,RFQHead_RFQNum desc ");
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        this.ddlExpired.SelectedValue = this.showexpired;
    ...
    #region 组合SQL查询语句==========================
    protected string CombSqlTxt(int _status, string _note_no, string _start_time, string _stop_time,string _showexpired)
    {
        ...
        if(_showexpired == "0")
            strTemp.Append(" and Calculated_RFQExpired='N' ");
        else if (_showexpired == "1")
            strTemp.Append(" and Calculated_RFQExpired='Y' ");
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        (new EpicorRequest()).GetEpicorRFQList(this.vendor_id);
        (new EpicorRequest()).GetEpicorRFQRFHead(this.vendor_id);
        Response.Redirect(Utils.CombUrlTxt("purchase_request2.aspx", "start_time={0}&stop_time={1}&note_no={2}&showexpired={3}", this.txtstart_time.Value, this.txtstop_time.Value, txtNote_no.Text, this.ddlExpired.SelectedValue));
    }
